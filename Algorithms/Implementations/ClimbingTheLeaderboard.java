...
public class ClimbingTheLeaderboard {
	...
	public static void main(String[] args) {
        ...
        for(int scores_i=0; scores_i < n; scores_i++){
            scores[scores_i][0] = in.nextInt();
            if(scores_i == 0) {
            	scores[scores_i][1] = rank;
            } else {
            	if(scores[scores_i][0] == scores[scores_i-1][0]) {
            		scores[scores_i][1] = rank;
            	} else {
            		rank++;
            		scores[scores_i][1] = rank;
        ...
        for(int alice_i=0; alice_i < m; alice_i++){
            ...
            System.out.println(solve(scores, alice[alice_i]));
        }
        in.close();
        // your code goes here
	...
	static int solve(int[][] scores, int alice) {
		int lower = 0;
		int upper = scores.length - 1;
		int aliceRank = 1;
		while (lower <= upper) {
			int middle = (lower + upper) / 2;
			if (alice == scores[middle][0]) {
				aliceRank = scores[middle][1];
				break;
			} else if (alice < scores[middle][0]) {
				aliceRank = scores[middle][1] + 1;
				lower = middle + 1;
			} else {
				upper = middle - 1;
			}
